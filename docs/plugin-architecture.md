...
| 4 | `EtherCATWorkbenchPlugin` | In progress | Process Data and Startup are editable; DC and full tree remain |
...
Workbench/Core extension points. Process Data and ordered Startup requests are
now editable for configured slaves through the public Project service; their
repository views stay read-only. DC is still read-only, and the tree does not
yet contain the required Inputs, Outputs, RxPDO, TxPDO, and Modules/Channels
